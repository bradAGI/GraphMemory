...
class NodeOutput(BaseModel):
    ...
    proper_noun: bool = Field(default=False, description="Whether the node is a proper noun.")
...
class NodesSignature(dspy.Signature):
    ...
    input_text: str = dspy.InputField(description="The input text to extract nodes from.")
...
class EdgesSignature(dspy.Signature):
    ...
    input_text: str = dspy.InputField(description="The unstructured input text to extract edges from.")
    input_nodes: list[Node] = dspy.InputField(description="The nodes in the graph to connect.")
    output_edges: list[EdgeOutput] = dspy.OutputField(description="The edge list connecting the nodes in the graph.")
...
for sentence in sentences:
    try:
        ...
        new_nodes = [
            Node(properties=node_dict.properties, type=node_dict.type)
            for node_dict in new_nodes_dicts
            if node_dict.properties and node_dict.proper_noun
        ]
...
for sentence in sentences:
    try:
        new_edges_dict = edges_predictor(input_text=sentence, input_nodes=nodes).output_edges
        new_edges = [
            Edge(source_id=edge.source_id, target_id=edge.target_id, relation=edge.relation) for edge in new_edges_dict
        ]
